...
//Isto e cute gostava de usar
typedef enum _type {INT, CHAR, VOID, SHORT, DOUBLE} type;

typedef struct _sf{
	type function_type;
        char* id;
	argument_linked_list* list_head;
}function_structure;

typedef struct _sal{
        char* type;
        argument_linked_list* next;
}argument_linked_list;

//Codigo da ficha, esta aqui para referencia
